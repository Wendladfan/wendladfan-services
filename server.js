...
// Utiliser /tmp pour Vercel (écriture autorisée)
const dbPath = process.env.VERCEL ? '/tmp/wendladfan.db' : path.join(__dirname, 'database', 'wendladfan.db');
if (!process.env.VERCEL && !fs.existsSync('./database')) fs.mkdirSync('./database');
...
async function initDB() {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS projects (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, category TEXT, description TEXT, media_type TEXT DEFAULT 'image', media_url TEXT, image_color TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS testimonials (id INTEGER PRIMARY KEY AUTOINCREMENT, author TEXT, content TEXT, stars INTEGER DEFAULT 5)`);
